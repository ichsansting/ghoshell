...
- [ ] `gho launch <fixture-vault>` prompts passphrase → decrypts via 02 → composes via 03.
- [ ] Sets `HOME` to a RAM-backed (tmpfs) session dir with a temp-dir fallback; materializes
      the composed files there; secret files written `0600`.
- [ ] Writes the composed `mise.toml` into the ephemeral HOME.
- [ ] Spawns the shell as `mise x -- fish` so tools lazy-install on first use and land on
      `PATH` for the shell and any process it spawns.
- [ ] On normal exit AND on signal (Ctrl-C / kill), tears down the session dir — structural
      wipe on linux/tmpfs; best-effort scrub-then-unlink of secret files on the mac fallback.
- [ ] Integration test drives launch against a fixture vault and asserts: ephemeral HOME
