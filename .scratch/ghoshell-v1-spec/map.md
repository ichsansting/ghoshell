...
- [06 — Multi-profile manifest schema](issues/06-multi-profile-manifest.md) — one manifest, top-level **`profiles:` map** (picker enumerates keys). A profile = a **composition of components** (not inheritance): a **component** bundles `{ mise specs + config files + secret files }`; a profile lists component names; launch materializes the **flat union**. **Disjointness rule** (TUI-enforced at author time): no same-path files, no conflicting tool versions within a profile — keeps it flat, no override (override = post-v1 fog). Shared secret (e.g. one SSH key) = a component included by many profiles, stored once. Names stay sealed (tool names visible on wire, per 05). **Tool versions float `@latest`** for v1 (pinning/lockfile = post-v1 ceiling, a new field not a reshape). **Tool mechanism: launch writes one composed `mise.toml` (`[tools]` = union of specs) and spawns the shell as `mise x -- fish`** — no shims; `auto_install` lazy-installs on first use, real bin dirs on `PATH` so apps that shell out see the tools (verified in-env with an uninstalled `rg`).

## Not yet specified

- **Profile inheritance / override** (surfaced by [03], sharpened by [06]) — v1 chose flat **composition** with a disjointness rule (no override). The deferred piece is now specifically: **override/selective-inheritance** — a component whose file *replaces* another's at the same path, or multi-level bases. Only matters once "some but not all" or "same file, tweaked per profile" becomes common. Post-v1.
- **mise reproducibility / version pinning** — RESOLVED by [06]: v1 floats `@latest`; pinning + `mise.lock`-in-vault is the documented post-v1 ceiling.
